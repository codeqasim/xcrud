...
function user_id_update($postdata, $primary, $xcrud){
    ...
    // $pass = $db->query('SELECT * FROM `users` WHERE id='.$primary.'')->fetch_object();
    // $password=($pass->password);

    $update = $db->query("UPDATE users SET user_id=".$user_id." WHERE id=".$primary."");
    // $update = $db->query("UPDATE users SET password=".md5($password)." WHERE id=".$primary."");

}

function lesson_id_update($postdata, $primary, $xcrud){
    ...
    $update = $db->query("UPDATE courses_lessons SET lesson_id=".$lesson_id." WHERE id=".$primary."");
